...
プロダクションにおいては、より堅牢な設計として、`recog_break` フラグを、この実装のようにキューと別に持つのではなく、キューのデータ要素自体がデータの終了等を示すことができるように、要素自体を short 型ではなく、short 型のサンプル値を含む何らかの構造体とするといった設計を検討してください。
