    ...
    connection_monitor_ = std::async(std::launch::async, [&] {
        sleep(60);
        {
            ...
            if (state_.load() == Stream::Status::connected) {
                ...
                if (param_.verbose_) {
                    std::cerr << "mimi speculative connection is closed because no speech is detected in 60 seconds."
                              << std::endl;
                }
            }
        }
...
void Stream::start() {
    ...
    {
        std::unique_lock<std::mutex>(mutex_);
        if (state_.load() == Stream::Status::connecting || state_.load() == Stream::Status::connected ||
            state_.load() == Stream::Status::connectionTimedout) {
            if (param_.verbose_) {
    ...
    start_future_ = std::async(std::launch::async, [&] {
        ...
        if (state_.load() == Stream::Status::connectionFailed) {
            ...
            if (param_.verbose_) {
                std::cerr
                        << "Previous speculative connection has been already expired. Try re-connecting to remote host..."
                        << std::endl;
            }
